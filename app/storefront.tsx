...
type Product = { id: number; name: string; kind: string; price: number; shade: string; image: string; badge?: string; description: string };

const products: Product[] = [
  { id: 1, name: "Velours Solaire", kind: "Eau de parfum · 50 ml", price: 86, shade: "Ambre chaud", badge: "Bestseller", image: "https://images.unsplash.com/photo-1541643600914-78b084683601?auto=format&fit=crop&w=900&q=85", description: "Un sillage d'ambre, de bergamote et de bois clair." },
  { id: 2, name: "Rituel N° 04", kind: "Huile visage · 30 ml", price: 54, shade: "Rose fumée", badge: "Nouveau", image: "https://images.unsplash.com/photo-1608248543803-ba4f8c70ae0b?auto=format&fit=crop&w=900&q=85", description: "Une huile soyeuse qui révèle l'éclat naturel." },
  { id: 3, name: "Satin Libre", kind: "Rouge à lèvres · 3,5 g", price: 29, shade: "Terra rose", image: "https://images.unsplash.com/photo-1586495777744-4413f21062fa?auto=format&fit=crop&w=900&q=85", description: "Une couleur intense et un confort seconde peau." },
  { id: 4, name: "Brume d'Atelier", kind: "Brume corps · 100 ml", price: 42, shade: "Néroli", image: "https://images.unsplash.com/photo-1619994403073-2cec844b8e63?auto=format&fit=crop&w=900&q=85", description: "La fraîcheur lumineuse d'un matin en Méditerranée." },
];
...
export function Storefront() {
  ...
  return <main>
    ...
    <header className="site-header">
      <a className="wordmark" href="#top" aria-label="Éclat, accueil">ÉCLAT<span>·</span></a>
      <nav aria-label="Navigation principale"><a href="#nouveautes">Nouveautés</a><a href="#rituels">Rituels</a><a href="#maison">La maison</a></nav>
    ...
    <section id="top" className="hero">
      <div className="hero-copy"><p className="eyebrow">La collection Équinoxe</p><h1>La lumière<br/><em>vous va si bien.</em></h1><p className="hero-text">Des soins et fragrances pensés comme des gestes de présence. Singuliers, sensuels, essentiels.</p><a href="#nouveautes" className="button button-dark">Découvrir la collection <span>↗</span></a></div>
      <div className="hero-visual"><div className="sun-disc"></div><div className="hero-image"></div><p className="vertical-note">FORMULÉ À PARIS · FABRIQUÉ EN FRANCE</p><p className="hero-caption">Équinoxe<br/><span>l'édition solaire</span></p></div>
    </section>

    <section id="nouveautes" className="products-section"><div className="section-head"><div><p className="eyebrow">À découvrir</p><h2>Les essentiels<br/>du moment.</h2></div><a href="#catalogue">Voir tout <span>→</span></a></div><div className="product-grid" id="catalogue">{shownProducts.map((product) => <article className="product-card" key={product.id}><button className="product-image" onClick={() => setActiveProduct(product)}><img src={product.image} alt={product.name}/>{product.badge && <span className="badge">{product.badge}</span>}<span className="quick">Voir le produit</span></button><div className="product-details"><div><h3>{product.name}</h3><p>{product.kind}</p></div><button onClick={() => add(product)} aria-label={`Ajouter ${product.name} au panier`}>+</button></div><div className="product-price"><span className="swatch" style={{background: product.id === 3 ? "#9d4e50" : product.id === 2 ? "#c89182" : "#d19b55"}}></span>{product.shade}<strong>{euro.format(product.price)}</strong></div></article>)}</div>{shownProducts.length === 0 && <p className="empty">Aucun produit ne correspond à cette recherche.</p>}</section>
    ...
    <section className="journal" id="maison"><p className="eyebrow">Le journal Éclat</p><div><h2>Des histoires<br/>à porter.</h2><a href="#newsletter" className="button button-light">Entrer dans le journal <span>↗</span></a></div></section>
    <footer id="newsletter"><div className="footer-top"><a className="wordmark" href="#top">ÉCLAT<span>·</span></a><div><h3>Un peu de lumière<br/>dans votre boîte mail.</h3><form onSubmit={(event) => event.preventDefault()}><input type="email" required placeholder="Votre adresse e-mail" aria-label="Votre adresse e-mail"/><button aria-label="S'inscrire">→</button></form></div></div><div className="footer-bottom"><span>© 2026 Éclat Paris</span><span>Livraison & retours · Conditions · Confidentialité</span><span>France / EUR</span></div></footer>
