...
export function ShopPage() {
  ...
  return <main className="shop-all">
    ...
    <header className="shop-header"><a className="shop-menu" href="/" aria-label="Retour à l'accueil">←</a><a href="/" className="shop-logo">Simire</a><div className="shop-tools"><label><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher" aria-label="Rechercher"/></label><button aria-label="Compte client">◯</button><button aria-label="Sac">Sac <b>0</b></button></div></header>
    ...
    <section className="shop-intro"><p className="eyebrow">La collection Simire</p><h1>Shop all</h1><p>Des essentiels de couleur et de soin, créés pour jouer, mélanger et rayonner à votre façon.</p></section>
    ...
    <section className="shop-newsletter"><p className="eyebrow">La liste Simire</p><h2>Les bonnes nouvelles<br/>vous iront bien.</h2><form onSubmit={(event) => event.preventDefault()}><input type="email" placeholder="Votre e-mail" aria-label="Votre e-mail"/><button>S'inscrire →</button></form></section>
  </main>;
}
