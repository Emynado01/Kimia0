...
export function ShopPage() {
  ...
  const addToCart = (product: Product) => { setCart((items) => [...items, product]); setJustAdded(product.name); if (navigator.vibrate) navigator.vibrate([30, 35, 45]); window.setTimeout(() => setJustAdded(null), 1200); };
  const remove = (index: number) => setCart((items) => items.filter((_, itemIndex) => itemIndex !== index));
  const checkout = () => { window.sessionStorage.setItem(CHECKOUT_DRAFT_KEY, JSON.stringify(createCheckoutDraft(cart))); router.push("/checkout"); };
  ...
  return <main className="shop-all">
    ...
    <section className="shop-grid">{visible.map((product) => <article className="shop-product" key={product.id}><div className="shop-product-image"><button className="shop-product-open" onClick={() => setActiveProduct(product)} aria-label={`Voir ${product.name}`}><img src={product.image} alt={product.name}/></button>{product.badge && <span>{product.badge}</span>}<button className={liked.includes(product.id) ? "heart saved" : "heart"} onClick={() => setLiked((items) => items.includes(product.id) ? items.filter((id) => id !== product.id) : [...items, product.id])} aria-label={`Ajouter ${product.name} aux favoris`}>{liked.includes(product.id) ? "♥" : "♡"}</button></div><div className="shop-product-info"><h2><button className="shop-product-title" onClick={() => setActiveProduct(product)}>{product.name}</button></h2><p className="shop-stars">★★★★★</p><p>{product.kind}</p><strong>{euro.format(product.price)}</strong><button className="shop-buy" onClick={() => addToCart(product)}>Ajouter au sac</button></div></article>)}</section>
